...
contract WesolowskiVerification {
    ...
    function naiveVerify(BigNumber memory a,BigNumber memory l,BigNumber memory pi,BigNumber memory t,BigNumber memory N,BigNumber memory ySaved) internal verifyParams(a,l,pi,t,N,ySaved) returns (bool) {
    ...
    remainder = BigNumbers.modexp(BigNumbers.two(),t,l);



    

    y = BigNumbers.modexp(pi,l,N);

    y_tmp = BigNumbers.modexp(a,remainder,N);

    y= BigNumbers.modmul(y,y_tmp,N);

    

    

    if (BigNumbers.eq(y,ySaved)) {
    ...
    function caller(string memory a,string memory l,string memory pi,string memory t,string memory N,string memory ySaved) public returns (bool){
        ...
        BigNumber memory one = BigNumbers.init(x,false,256);

        BigNumber memory two= BigNumbers.init(bytes(l),false,256);

        BigNumber memory three= BigNumbers.init(bytes(pi),false,256);

        BigNumber memory four= BigNumbers.init(bytes(t),false,256);

        BigNumber memory five= BigNumbers.init(bytes(N),false,256);

        BigNumber memory six= BigNumbers.init(bytes(ySaved),false,256);

        if(naiveVerify(one, two, three, four, five, six)){

        // emit Verified();

        // emit VerificationEnded();
